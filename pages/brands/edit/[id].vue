<template>
  <v-card class="spike-card" elevation="10">
    <v-card-item>
      <h5 class="text-20 mb-7">ویرایش برند</h5>
      ...
      <v-form ref="formRef" v-model="valid" validate-on="submit lazy" @submit.prevent="submitForm"
        ...
        <div class="d-flex align-center justify-end ga-3">
          ...
          <v-btn flat color="error" to="/brands" class="mt-6" rounded="pill">
            انصراف
          </v-btn>
        </div>
      </v-form>
...
const getBrand = async () => {
  ...
  try {
    ...
    const data = await useApiService.get(`brands/${brandId}`);
    form.value = {
      _id: data._id,
      title: data.title
    };
  } catch (error) {
    $notify('مشکلی در بارگذاری اطلاعات برند پیش آمد', 'error');
    router.push('/brands');
  } finally {
    loading.value = false;
  }
...
const submitForm = async () => {
  ...
  if (isValid) {
    ...
    try {
      await useApiService.put(`brands/${form.value._id}`, {
        title: form.value.title
      });
      $notify('برند با موفقیت ویرایش شد', 'success');
      router.push('/brands');
    } catch (error) {
      $notify('مشکلی در ویرایش برند پیش آمد', 'error');
    } finally {
