...
const getAccounts = async () => {
  loading.value = true;
  try {
    const data = await useApiService.get("accounts?" + filter());
...
const deleteAccount = async (_id) => {
  try {
    await useApiService.remove("accounts/" + _id);
...
const setDefault = async (data) => {
  try {
    await useApiService.put("accounts/default/" + data._id, {});
    $notify("عملیات با موفقیت انجام شد", "success");
    await getAccounts();
  } catch (error) {
    $notify("مشکلی در عملیات پیش آمد؛ لطفا دوباره تلاش کنید", "error");
