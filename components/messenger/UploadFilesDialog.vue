  ...
  <v-dialog width="350">
    <v-card class="rounded-lg">
      ...
      <div class="pb-0 px-5 mt-0 mb-3 py-0 d-flex flex-column w-100">
        ...
        <!--    Text     -->
        <v-text-field class="rounded-0 py-0"
                      v-model="text"
                      variant="underlined"
...
const uploadFile = async (file) => {
  ...
  await $axios.post('conversations/' + props._conversation + '/files', data, {
...
const sendTextMessage = async (content) => {
  ...
  if (content && props._conversation) {
    await useAPI('conversations/' + props._conversation + '/messages', {
      method: 'post',
      body  : {
        type   : 'text',
        content: content
...
watch(() => props._conversation, (newValue, oldValue) => {
