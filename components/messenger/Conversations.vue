<template>
  <div>
    ...
    <v-row class="d-flex border pt-2 pb-2 mb-0 px-4 mx-0">
      <!--    Menu Button    -->
      <v-btn class="mr-1 ml-2" @click="" variant="plain" icon>
      ...
      <v-label v-if="loading && listAction === 'conversations'"
        >در حال به روز رسانی...</v-label
      >

      <v-label v-if="listAction === 'conversations'"> پیام رسان </v-label>

      <!--      Search      -->
      <v-text-field
        v-if="listAction === 'search'"
    ...
    <!--  Empty List   -->
    <v-row
      v-if="!Object.values(listOfConversations).length && !loading"
      class="h-100 text-subtitle-1"
    >
      <v-col cols="12" class="text-center my-0">هیچ گفتگویی ندارید</v-col>
      <v-col cols="12" class="text-center my-n5"
        >با انتخاب مخاطب خود گفتگویی را شروع کنید</v-col
      >
      <v-col cols="12" class="text-center">
        <v-btn
          class="mt-5 mb-2 rounded-xl"
          prepend-icon="mdi-account-outline"
          @click="goToContacts"
          color="secondary"
        >
    ...
    <v-list class="listHeight mt-0 pb-5 mb-0 overflow-auto">
      <v-list-item
        v-for="(conversation, i) in listOfConversations"
      ...
      >
        ...
        <v-list-item-subtitle
          v-if="getConversationLastMessage(conversation)"
          class="w-100"
        >
          ...
          <span
            v-if="getConversationLastMessage(conversation).type === 'image'"
          >
            تصویر
          </span>
          <span
            v-if="getConversationLastMessage(conversation).type === 'video'"
          >
            ویدئو
          </span>
          <span
            v-if="getConversationLastMessage(conversation).type === 'audio'"
          >
            صدا
          </span>
          ...
          <v-row class="d-inline-block my-0 py-0">
            ...
            <v-col class="my-0 py-0 d-flex justify-center" cols="12">
              <label
                ...
                class="unreadCount bg-secondary"
              >
    ...
    <DeleteConversationDialog
      v-model="deleteConversationDialog.show"
...
const getConversationLastMessage = (conversation) => {
  if (
    messengerStore.messages[conversation._id] &&
    Object.values(messengerStore.messages[conversation._id]).length
  ) {
    return Object.values(messengerStore.messages[conversation._id]).reduce(
      (latest, current) => {
...
const openConversationContextMenu = (event, conversationId) => {
  ...
  conversationContextMenu.value.style = {
...
const getConversations = async () => {
  ...
  try {
    const data = await useApiService.get("conversations");
    if (data) {
      // clear the conversations
      messengerStore.clearConversations();

      data.list.forEach((conversation) => {
