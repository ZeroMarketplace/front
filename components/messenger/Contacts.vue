<template>
  <div>
    ...
    <AddContact
      ...
      @refresh="getContacts"
      @exit="addContactDialog = false"
    />

    <!--   Search And Back    -->
    <v-row class="d-flex border pt-2 pb-2 mb-0 px-4 mx-0">
      <!--    Menu Button    -->
      <v-btn class="mr-1 ml-2" @click="closeContactsList" variant="plain" icon>
        <v-icon>mdi-arrow-right</v-icon>
      </v-btn>

      <!--   Search   -->
      <v-text-field
        class="ml-2 mt-1 mb-0 d-block"
        v-if="searchFlag"
        v-model="search"
      ...
      <v-label v-if="!searchFlag" class=""> مخاطبین </v-label>
    ...
    <!--  Empty List   -->
    <v-row
      v-if="!list.length && !loading && !searchFlag"
    ...
    >
      ...
      <v-col cols="12" class="text-center my-n5"
        >میتوانید با کلیک روی دکمه زیر مخاطب اضافه کنید</v-col
      >
  ...
  if (search.value) {
    ...
    return contacts.value.filter((user) => {
      ...
      return (
        ...
        (names.length > 1 &&
          firstName.includes(names[0].toLowerCase()) &&
          lastName.includes(names.slice(1).join(" ").toLowerCase())) ||
        // Search assuming the first word is in `last` and the rest in `first`
        (names.length > 1 &&
          lastName.includes(names[0].toLowerCase()) &&
          firstName.includes(names.slice(1).join(" ").toLowerCase()))
      );
    });
  } else {
...
const getContacts = async () => {
  ...
  try {
    const data = await useApiService.get("contacts");
    if (data) {
